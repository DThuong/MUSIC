<?php 
	if($action == 'add')
	{
		...
		if($_SERVER['REQUEST_METHOD'] == "POST")
		{
			...
			else if(!preg_match("/^(?=.*[a-zA-Z])(?=.*[\-&ÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẢỀỀỄỄỂỂỄỄịọọởởỠỠỢỢũũưứửữữửỬỬỮỮỰỰỲỲỴỴỵỵ\s]*).*$/u", $_POST['name'])){
				$errors['name'] = "Name can contain only letters, hyphen, ampersand, spaces and dot";
			}
			//image
	...
	if($action == 'edit')
	{
		...
		if($_SERVER['REQUEST_METHOD'] == "POST" && $row)
		{
			...
			if(!preg_match("/^(?=.*[a-zA-Z])(?=.*[\-&ÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẢỀỀỄỄỂỂỄỄịọọởởỠỠỢỢũũưứửữữửỬỬỮỮỰỰỲỲỴỴỵỵ\s]*).*$/u", $_POST['name'])){
				$errors['name'] = "Name can contain only letters, hyphen, ampersand, spaces and dot";
			}
